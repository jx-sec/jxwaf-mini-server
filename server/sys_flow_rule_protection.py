...
def waf_get_sys_flow_rule_protection_list(request):
    return_result = {}
    data = []
    try:
        ...
        if rule_type == "single_rule":
            ...
            results = sys_flow_rule_protection.objects.filter(user_id=user_id).filter(rule_type=rule_type).filter(
...
def waf_get_sys_flow_rule_protection(request):
    return_result = {}
    data = {}
    try:
        user_id = request.session['user_id']
        ...
        result = sys_flow_rule_protection.objects.get(Q(user_id=user_id) & Q(rule_uuid=rule_uuid))
...
def waf_del_sys_flow_rule_protection(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        rule_uuid = json_data['rule_uuid']
        try:
            sys_flow_rule_protection.objects.filter(user_id=user_id).filter(rule_uuid=rule_uuid).delete()
            return_result['result'] = True
...
def waf_edit_sys_flow_rule_protection(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            sys_flow_rule_protection.objects.filter(rule_uuid=rule_uuid).filter(user_id=user_id).update(
...
def waf_create_sys_flow_rule_protection(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        if rule_count != 0:
            ...
            return JsonResponse(return_result, safe=False)
        if rule_type == "single_rule":
            sys_flow_rule_protection.objects.create(user_id=user_id, rule_name=rule_name, rule_detail=rule_detail,
                                                    rule_matchs=rule_matchs, rule_action=rule_action,
                                                    action_value=action_value, rule_log=rule_log,
                                                    update_time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        elif rule_type == "group_rule":
            rule_group_uuid = json_data["rule_group_uuid"]
            sys_flow_rule_protection_group_result = sys_flow_rule_protection_group.objects.get(
            ...
            sys_flow_rule_protection.objects.create(user_id=user_id, rule_name=rule_name, rule_detail=rule_detail,
                                                    ...
                                                    rule_group_name=sys_flow_rule_protection_group_result.rule_group_name,
...
def waf_search_sys_flow_rule_protection(request):
    return_result = {}
    data = []
    try:
        user_id = request.session['user_id']
        ...
        if search_type == "rule_uuid":
            results = sys_flow_rule_protection.objects.filter(user_id=user_id).filter(rule_type="single_rule").filter(
                rule_uuid__contains=search_value)
        elif search_type == "rule_name":
            results = sys_flow_rule_protection.objects.filter(user_id=user_id).filter(rule_type="single_rule").filter(
                rule_name__contains=search_value)
        else:
            results = sys_flow_rule_protection.objects.filter(user_id=user_id).filter(rule_type="single_rule")
        for result in results:
            data.append({'rule_uuid': result.rule_uuid,
                         'rule_name': result.rule_name,
                         'rule_detail': result.rule_detail,
