...
def waf_get_sys_web_white_rule_group_list(request):
    return_result = {}
    data = []
    try:
        user_id = request.session['user_id']
        results = sys_web_white_rule_group.objects.filter(user_id=user_id)
        for result in results:
            sys_web_white_rule_count = sys_web_white_rule.objects.filter(user_id=user_id).filter(
            ...
            data.append({'rule_group_uuid': result.rule_group_uuid,
                         ...
                         'rule_count': sys_web_white_rule_count,
...
def waf_del_sys_web_white_rule_group(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        ...
        try:
            ...
            if sys_web_white_rule_count > 0:
                return_result['result'] = False
                return_result['message'] = 'del error,exist rule'
                return JsonResponse(return_result, safe=False)
            sys_web_white_rule_group.objects.filter(user_id=user_id).filter(
                rule_group_uuid=rule_group_uuid).delete()
            sys_web_white_rule.objects.filter(user_id=user_id).filter(
                rule_group_uuid=rule_group_uuid).delete()
            return_result['result'] = True
...
def waf_edit_sys_web_white_rule_group(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            sys_web_white_rule_group.objects.filter(rule_group_uuid=rule_group_uuid).filter(
...
def waf_create_sys_web_white_rule_group(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            ...
            if len(result) == 0:
                sys_web_white_rule_group.objects.create(user_id=user_id, rule_group_name=rule_group_name,
                                                        rule_group_detail=rule_group_detail)
            else:
                return_result['result'] = False
...
def waf_search_sys_web_white_rule_group(request):
    return_result = {}
    data = []
    try:
        user_id = request.session['user_id']
        ...
        if search_type == "rule_group_uuid":
            results = sys_web_white_rule_group.objects.filter(user_id=user_id).filter(
            ...
            results = sys_web_white_rule_group.objects.filter(user_id=user_id).filter(rule_type="single_rule")
...
def waf_get_sys_web_white_rule_group(request):
    return_result = {}
    data = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        rule_group_uuid = json_data['rule_group_uuid']
        result = sys_web_white_rule_group.objects.get(Q(user_id=user_id) & Q(rule_group_uuid=rule_group_uuid))
