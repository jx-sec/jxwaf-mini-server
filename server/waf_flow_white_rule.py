...
def waf_get_flow_white_rule_list(request):
    ...
    try:
        ...
        waf_flow_white_rule_results = waf_flow_white_rule.objects.filter(user_id=user_id).filter(
            domain=domain).order_by('rule_order_time')
        for result in waf_flow_white_rule_results:
    ...
    try:
        ...
        try:
            if rule_type == "single_rule":
                ...
                results = waf_flow_white_rule.objects.filter(user_id=user_id).filter(uuid=rule_uuid)
                if len(results) == 0:
                    waf_flow_white_rule.objects.create(user_id=user_id, domain=domain, uuid=rule_uuid,
...
def waf_exchange_flow_white_rule_priority(request):
    ...
    try:
        ...
        if type == "top":
            rule_uuid = json_data['rule_uuid']
            waf_flow_white_rule_results = waf_flow_white_rule.objects.filter(domain=domain).filter(
                user_id=user_id).order_by('rule_order_time')
            waf_flow_white_rule_result = waf_flow_white_rule_results[0]
            waf_flow_white_rule.objects.filter(domain=domain).filter(user_id=user_id).filter(uuid=rule_uuid).update(
                rule_order_time=int(waf_flow_white_rule_result.rule_order_time) - 1)
        elif type == "exchange":
            rule_uuid = json_data['rule_uuid']
            exchange_rule_uuid = json_data['exchange_rule_uuid']
            rule_uuid_result = waf_flow_white_rule.objects.get(
                Q(domain=domain) & Q(user_id=user_id) & Q(uuid=rule_uuid))
            exchange_rule_uuid_result = waf_flow_white_rule.objects.get(
