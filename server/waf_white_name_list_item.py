...
def waf_get_white_name_list_item_list(request):
    return_result = {}
    data = []
    try:
        ...
        try:
            ...
            page = 1
        results = waf_white_name_list_item.objects.filter(user_id=user_id).filter(domain=domain).filter(
...
def waf_del_white_name_list_item(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        domain = json_data['domain']
        try:
            waf_white_name_list_item.objects.filter(user_id=user_id).filter(name_list_name=name_list_name).filter(
...
def waf_create_white_name_list_item(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
...
def waf_search_white_name_list_item(request):
    return_result = {}
    data = []
    try:
        user_id = request.session['user_id']
