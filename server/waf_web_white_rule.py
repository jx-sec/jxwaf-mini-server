...
def waf_get_web_white_rule_list(request):
    ...
    try:
        ...
        results = waf_web_white_rule.objects.filter(user_id=user_id).filter(
...
def waf_edit_web_white_rule_status(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            waf_web_white_rule.objects.filter(rule_name=rule_name).filter(user_id=user_id).filter(
...
def waf_get_web_white_rule(request):
    return_result = {}
    data = {}
    try:
        user_id = request.session['user_id']
        ...
        domain = json_data['domain']
        rule_name = json_data['rule_name']
        result = waf_web_white_rule.objects.get(Q(user_id=user_id) & Q(domain=domain) & Q(rule_name=rule_name))
...
def waf_create_web_white_rule(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        rule_action = json_data['rule_action']
        action_value = json_data['action_value']
        rule_count = waf_web_white_rule.objects.filter(user_id=user_id).filter(domain=domain).filter(
            rule_name=rule_name).count()
        if rule_count != 0:
            return_result['message'] = 'already_exists_rule'
            return_result['result'] = False
            return JsonResponse(return_result, safe=False)
        waf_web_white_rule.objects.create(user_id=user_id, rule_name=rule_name, rule_detail=rule_detail,
...
def waf_exchange_web_white_rule_priority(request):
    ...
    try:
        ...
        if type == "top":
            rule_name = json_data['rule_name']
            results = waf_web_white_rule.objects.filter(domain=domain).filter(
            ...
            rule_name_result = waf_web_white_rule.objects.get(
                Q(domain=domain) & Q(user_id=user_id) & Q(rule_name=rule_name))
            exchange_rule_name_result = waf_web_white_rule.objects.get(
...
def waf_backup_web_white_rule(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
