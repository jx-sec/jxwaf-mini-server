...
def waf_get_white_name_list_list(request):
    return_result = {}
    data = []
    try:
        ...
        results = waf_white_name_list.objects.filter(user_id=user_id).filter(domain=domain).order_by('order_time')
        for result in results:
            count = waf_white_name_list_item.objects.filter(user_id=user_id).filter(domain=domain).filter(
...
def waf_get_white_name_list(request):
    return_result = {}
    data = {}
    try:
        user_id = request.session['user_id']
        ...
        result = waf_white_name_list.objects.get(
            Q(user_id=user_id) & Q(name_list_name=name_list_name) & Q(domain=domain))
...
def waf_del_white_name_list(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        name_list_name = json_data['name_list_name']
        try:
            waf_white_name_list.objects.filter(user_id=user_id).filter(name_list_name=name_list_name).filter(
...
def waf_edit_white_name_list(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            waf_white_name_list.objects.filter(name_list_name=name_list_name).filter(user_id=user_id).filter(
...
def waf_edit_white_name_list_status(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
...
def waf_create_white_name_list(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        name_list_expire_time = json_data['name_list_expire_time']
        domain = json_data['domain']
        count = waf_white_name_list.objects.filter(user_id=user_id).filter(name_list_name=name_list_name).filter(
            domain=domain).count()
        if count > 0:
            return_result['result'] = False
            return_result['message'] = 'create error,name_list is exist'
            return JsonResponse(return_result, safe=False)
        waf_white_name_list.objects.create(user_id=user_id, name_list_name=name_list_name,
                                           name_list_detail=name_list_detail,
                                           name_list_rule=name_list_rule, name_list_action=name_list_action,
                                           action_value=action_value, domain=domain, order_time=int(time.time()),
...
def waf_exchange_white_name_list_priority(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        if type == "top":
            name_list_name = json_data['name_list_name']
            results = waf_white_name_list.objects.filter(
                user_id=user_id).filter(domain=domain).order_by('order_time')
            result = results[0]
            waf_white_name_list.objects.filter(user_id=user_id).filter(domain=domain).filter(
                name_list_name=name_list_name).update(
                order_time=int(result.order_time) - 1)
        elif type == "exchange":
            name_list_name = json_data['name_list_name']
            exchange_name = json_data['exchange_name']
            name_list_result = waf_white_name_list.objects.get(
                Q(user_id=user_id) & Q(name_list_name=name_list_name))
            exchange_name_list__result = waf_white_name_list.objects.get(
                Q(user_id=user_id) & Q(name_list_name=exchange_name))
            waf_white_name_list.objects.filter(user_id=user_id).filter(
