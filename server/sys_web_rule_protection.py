    ...
    try:
        ...
        if rule_type == "single_rule":
            ...
            for result in results:
                waf_domain_count = waf_web_rule_protection.objects.filter(user_id=user_id).filter(uuid).distinct('domain').count()
                waf_group_domain_count = waf_group_web_rule_protection.objects.filter(user_id=user_id).filter(uuid).distinct('group_id').count()
