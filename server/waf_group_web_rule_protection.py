...
def waf_get_group_web_rule_list(request):
    return_result = {}
    data = []
    try:
        ...
        waf_group_web_rule_protection_results = waf_group_web_rule_protection.objects.filter(user_id=user_id).filter(
            group_id=group_id).order_by('rule_order_time')
        for result in waf_group_web_rule_protection_results:
...
def waf_del_group_web_rule(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            waf_group_web_rule_protection.objects.filter(user_id=user_id).filter(group_id=group_id).filter(
...
def waf_load_group_web_rule(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            if rule_type == "single_rule":
                ...
                results = waf_group_web_rule_protection.objects.filter(user_id=user_id).filter(uuid=rule_uuid)
                if len(results) == 0:
                    waf_group_web_rule_protection.objects.create(user_id=user_id, group_id=group_id, uuid=rule_uuid,
...
def waf_edit_group_web_rule(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            waf_group_web_rule_protection.objects.filter(group_id=group_id).filter(user_id=user_id).filter(
...
def waf_exchange_group_web_rule_priority(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        if type == "top":
            rule_uuid = json_data['rule_uuid']
            waf_web_rule_protection_results = waf_group_web_rule_protection.objects.filter(group_id=group_id).filter(
                user_id=user_id).order_by('rule_order_time')
            waf_web_rule_protection_result = waf_web_rule_protection_results[0]
            waf_group_web_rule_protection.objects.filter(group_id=group_id).filter(user_id=user_id).filter(uuid=rule_uuid).update(
                rule_order_time=int(waf_web_rule_protection_result.rule_order_time) - 1)
        elif type == "exchange":
            rule_uuid = json_data['rule_uuid']
            exchange_rule_uuid = json_data['exchange_rule_uuid']
            rule_uuid_result = waf_group_web_rule_protection.objects.get(
                Q(group_id=group_id) & Q(user_id=user_id) & Q(uuid=rule_uuid))
            exchange_rule_uuid_result = waf_group_web_rule_protection.objects.get(
