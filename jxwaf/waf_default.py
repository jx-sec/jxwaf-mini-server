...
def waf_create_default_domain(request):
    return_result = {}
    try:
...
def waf_edit_default_domain(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
...
def waf_get_default_domain(request):
    return_result = {}
    data = {}
    try:
        user_id = request.session['user_id']
...
def waf_get_default_config(request):
    return_result = {}
    data = {}
    try:
        user_id = request.session['user_id']
        ...
        try:
            waf_default_config.objects.get(Q(user_id=user_id))
        except:
            waf_default_config.objects.create(
                user_id=user_id, type='false', owasp_code='404', owasp_html='')

        waf_domain_result = waf_default_config.objects.get(Q(user_id=user_id))

        data['type'] = waf_domain_result.type
        data['owasp_code'] = waf_domain_result.owasp_code
        data['owasp_html'] = waf_domain_result.owasp_html
        return_result['result'] = True
        return_result['message'] = data
        return JsonResponse(return_result, safe=False)
    except Exception, e:
...
def waf_create_default_config(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            ...
            type = "false"
        try:
            result = waf_default_config.objects.filter(user_id=user_id)
            if len(result) != 0:
                return_result['result'] = False
                return_result['errCode'] = 409
                return_result['message'] = 'create error,domain is exist'
                ...
                waf_default_config.objects.create(
                    user_id=user_id, type=type, owasp_code=owasp_code, owasp_html=owasp_html)
                return_result['result'] = True
                return_result['message'] = 'create success'
                return JsonResponse(return_result, safe=False)
        except Exception, e:
...
def waf_edit_default_config(request):
    return_result = {}
    try:
        user_id = request.session['user_id']
        json_data = json.loads(request.body)
        ...
        try:
            waf_default_config.objects.filter(user_id=user_id).update(
                user_id=user_id, type=type, owasp_code=owasp_code, owasp_html=owasp_html)
            return_result['result'] = True
            return_result['message'] = 'edit success'
            return JsonResponse(return_result, safe=False)
